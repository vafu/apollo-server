...
pub const ART_CACHE_DIR: &str = "/tmp/art_cache";
...
impl SessionManager {
    pub fn new(tcp_server: Arc<TcpServer>) -> Self {
        Self {
            ...
            state: Arc::new(Mutex::new(UnifiedState {
                player_state: "stopped".to_string(),
    ...
    pub async fn update_transport_state(&self, player_name: &str, transport_state: &str) {
        let transport_state = transport_state.to_lowercase();
        let state_to_send = {
            ...
            state.player_state = transport_state.clone();
    ...
    async fn process_and_cache_art(&self, songid: String, remote_cover_url: String) -> Result<()> {
        let (exists, relative_url, cache_filepath) = art_paths(&remote_cover_url);
        if exists {
        ...
        fs::create_dir_all(ART_CACHE_DIR).context("creating art cache directory")?;
...
fn art_paths(remote_cover_url: &str) -> (bool, String, PathBuf) {
    let digest = md5::compute(remote_cover_url.as_bytes());
    let cache_filename = format!("{digest:x}.jpg");
    let cache_filepath = Path::new(ART_CACHE_DIR).join(&cache_filename);
    (
